			...
			<nav id="menu">
				<ul class="links">
					...
					<?php
						...
						{
					?>
							<li><a href="interview.php">資料顯示((面試考官使用))</a></li>
					<?php
						}
					?>
				</ul>
							...
							<div class="table-wrapper" style="color:black;width:100%;">
								<form method="post" style="width:100%;" action="">
									...
									<div class="select-wrapper">
										<select name="class"style="width:40%;float:left">
											<option value="T"<?php if($_POST['class']=="T") echo "selected"?>>T</option>
										</select>
										<select name="time"style="width:40%;float:left">
									...
									-->
										<button class="button special small" style="float:left;width:10%;padding:0 0.5em" name="RefreshData">查詢</button>
									</div>
									</form>
								<form method="post" style="width:100%;" action="interviewsent.php">
									<table class="alt">
										...
										<tbody>
											<tr>
													<th style="text-align: center;width:10%;"><div style="overflow:auto;width:100%;height:120px;">准考證號<br>姓名<br>梯次場次</div></th>
										...
										<?php
											...
											while ($DATA_rowres = mysqli_fetch_array($DATA_result_status, MYSQLI_BOTH))
											{
												...
												<tr>
													<td rowspan="1" style="background-color:<?php echo $Color?>"><?php echo $DATA_rowres[0]."<br>".$DATA_rowres[3]."<br>".$DATA_rowres[4]."-".$DATA_rowres[5]."-".$DATA_rowres[7]."(".$DATA_rowres[5].")"?></td>
													<?php
													if($_COOKIE['Uname']=='nttuadmin'||$_COOKIE['Uname']=='nttucsieadmin2'){
													?>
